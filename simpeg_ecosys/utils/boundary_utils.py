"""
Copy from simpeg repository:
https://github.com/simpeg/simpeg/blob/master/SimPEG/electromagnetics/static/resistivity/boundary_utils.py
"""
import numpy as np
...
def getxBCyBC_CC(mesh, alpha, beta, gamma):
    ...
    if mesh.dim == 1:  # 1D
        if len(alpha) != 2 or len(beta) != 2 or len(gamma) != 2:
            raise Exception("Length of list, alpha should be 2")
        ...
        b_xm = (0.5 * alpha_xm + beta_xm / h_xm) / \
            (0.5 * alpha_xm - beta_xm / h_xm)
        a_xp = gamma_xp / (0.5 * alpha_xp - beta_xp / h_xp)
        b_xp = (0.5 * alpha_xp + beta_xp / h_xp) / \
            (0.5 * alpha_xp - beta_xp / h_xp)
        ...
        if len(alpha) != 4 or len(beta) != 4 or len(gamma) != 4:
            raise Exception("Length of list, alpha should be 4")
        ...
        b_ym = (0.5 * alpha_ym + beta_ym / h_ym) / \
            (0.5 * alpha_ym - beta_ym / h_ym)
        a_yp = gamma_yp / (0.5 * alpha_yp - beta_yp / h_yp)
        b_yp = (0.5 * alpha_yp + beta_yp / h_yp) / \
            (0.5 * alpha_yp - beta_yp / h_yp)
        ...
        b_zm = (0.5 * alpha_zm + beta_zm / h_zm) / \
            (0.5 * alpha_zm - beta_zm / h_zm)
        a_zp = gamma_zp / (0.5 * alpha_zp - beta_zp / h_zp)
        b_zp = (0.5 * alpha_zp + beta_zp / h_zp) / \
            (0.5 * alpha_zp - beta_zp / h_zp)
